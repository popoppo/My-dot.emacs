...
(defvar *gtags-current-buffer-alist* nil
  "Current buffer alist.")
(defvar *gtags-buffer-stack-alist* nil
  "Stack list for tag browsing.")
(defvar *gtags-point-stack-alist* nil
  "Stack list for tag browsing.")
(defvar *gtags-history-list-alist* nil
  "Gtags history list.")
  
(defvar *gtags-context-alist* nil
  "Association list for context-list.")
;; context-alist :: (context-list1 context-list2 ... )
;; context-list  :: (window-number . context-stack)
;; context-stack :: (buf1 buf2 buf3 ...)
;; e.g. ((1 . (A B C)) (2 . (B D A)) (5 . (Z)))
...
(defun gtags-push-context ()
  ...
  (gtags-buffer-stack-push (current-buffer))
  (gtags-point-stack-push (point)))

;; pop context from stack
(defun gtags-pop-context ()
  (if (not (gtags-ref-buffer-stack)) nil
    (let (buffer point)
      ...
      (setq buffer (gtags-buffer-stack-pop))
      (setq point  (gtags-point-stack-pop))
      (list buffer point))))

;; if the buffer exist in the stack
(defun gtags-exist-in-stack (buffer)
  ;(memq buffer gtags-buffer-stack))
  (let ((result-lst (mapcar (lambda (arg) 
                              (memq buffer (cadr arg)))
                            *gtags-buffer-stack-alist*)))
    (let ((bool nil))
      (dolist (arg result-lst bool)
        (if arg (setq bool t))))))
;  t)
...
(defun gtags-pop-stack ()
  ...
  (let (delete context buffer)
    (if (and (not (equal (gtags-current-buffer) nil))
      ...
      (if (and (not (gtags-exist-in-stack (current-buffer)))
               (not (gtags-used-now (current-buffer))))
	  (setq delete t))
	  ...
	  (message "The tags stack is empty.")
        ...
        (gtags-save-current-buffer (current-buffer))
...
(defun current-window-number () 
  (interactive)
  (if (boundp 'win:current-config)
      win:current-config
    (selected-window)))

(defun gtags-ref-context-list (win-num alist)
  (assq win-num alist))

(defun gtags-ref-context-stack (win-num alist)
  (cadr (gtags-ref-context-list win-num alist)))

(defun gtags-push-context-alist (win-num alist context)
  (if (gtags-ref-context-list win-num alist)
      (setcdr (gtags-ref-context-list win-num alist)
              (list 
               (nconc-as-list (gtags-ref-context-stack win-num alist) 
                              context)))
    (let ((new-alist (list win-num (list context))))
      (setq alist 
            (if (null alist) 
                (list new-alist)
              (nconc  (list new-alist) alist)))))
  alist)

(defun nconc-as-list (lst elm)
  (nconc (list elm) lst))

(defun gtags-pop-context-alist (win-num alist)
  (let ((context-stack (gtags-ref-context-stack win-num alist)))
    (setq buf (cond
               ((null context-stack)
                nil)
               ((= (length context-stack) 1)
                (setq alist
                      (if (= (length alist) 1) 
                          nil
                        (delq (gtags-ref-context-list win-num alist) alist)))
                (car context-stack))
               (t
                (setcdr (gtags-ref-context-list win-num alist) 
                        (list (cdr context-stack)))
                (car context-stack))))
    (values buf alist)))

(defun gtags-used-now (buffer)
  (let ((result-lst (mapcar (lambda (arg) 
                              (equal buffer (cadr arg)))
                            *gtags-current-buffer-alist*)))
    (let ((non-nil-lst (gtags-filter '(lambda (elm)
                                        (if elm elm))
                                     result-lst)))
      (< 1 (length non-nil-lst)))))
...
; mod funcs
(defun gtags-current-buffer-list ()
  (assq (current-window-number) *gtags-current-buffer-alist*))

(defun gtags-current-buffer ()
  (cadr (assq (current-window-number) *gtags-current-buffer-alist*)))

(defun gtags-save-current-buffer (buf)
  (let ((win-num (current-window-number))
        (current-buf-list (gtags-current-buffer-list)))
    (if (null current-buf-list)
        (setq *gtags-current-buffer-alist* 
              (if (null *gtags-current-buffer-alist*)
                  (list `(,win-num ,buf))
                (append *gtags-current-buffer-alist*
                        (list `(,win-num ,buf)))))
      (setcdr (assq win-num 
                    *gtags-current-buffer-alist*) (list buf)))))

(defun gtags-ref-buffer-stack ()
  (gtags-ref-context-list (current-window-number) 
                          *gtags-buffer-stack-alist*))

(defun gtags-buffer-stack-push (context)
  (setq *gtags-buffer-stack-alist*
        (gtags-push-context-alist (current-window-number) 
                                  *gtags-buffer-stack-alist*
                                  context)))

(defun gtags-buffer-stack-pop ()
  (setq vals (gtags-pop-context-alist (current-window-number) 
                                      *gtags-buffer-stack-alist*))
  (setq *gtags-buffer-stack-alist* (cadr vals))
  (car vals))

(defun gtags-point-stack ()
  (gtags-ref-context-list (current-window-number) 
                          *gtags-point-stack-alist*))

(defun gtags-point-stack-push (point)
  (setq *gtags-point-stack-alist*
        (gtags-push-context-alist (current-window-number) 
                                  *gtags-point-stack-alist*
                                  point)))
(defun gtags-point-stack-pop ()
  (setq vals (gtags-pop-context-alist (current-window-number) 
                                      *gtags-point-stack-alist*))
  (setq *gtags-point-stack-alist* (cadr vals))
  (car vals))

(defun gtags-filter (f lst)
  (delq nil (mapcar f lst)))
